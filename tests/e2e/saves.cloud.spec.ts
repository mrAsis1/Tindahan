...
async function cloud(
  ...
  failure: "lost" | "before" | "refresh" | "none",
) {
  const state: StoreData = {
    version: 1,
    store: {
      id: "22222222-2222-4222-8222-222222222222",
      name: "Fictional test store",
    },
    customers: [
      {
        id: customer,
  ...
  await context.route("https://auth-test.supabase.co/**", async (route) => {
    ...
    if (
      path === "/rest/v1/rpc/get_notebook" ||
      path === "/rest/v1/rpc/read_notebook"
    ) {
      calls.reads++;
    ...
    if (path === "/rest/v1/rpc/record_entry") {
      ...
      const existing = state.entries.find(
        (entry) => entry.id === input.p_request_id,
      );
      ...
      const available = state.entries
        ...
        .reduce(
          (sum, entry) =>
            sum + (entry.type === "payment" ? -1 : 1) * entry.amountCentavos,
          0,
        );
      if (input.p_type === "payment" && input.p_amount_centavos > available)
        return json(
          { message: "Payment exceeds the available balance.", code: "P0001" },
          400,
        );
...
for (const failure of ["lost", "refresh"] as const) {
  test(`customer edits survive a ${failure} response without a second change`, async ({
    ...
    await page
      .getByRole("button", { name: "Save changes", exact: true })
      .click();
    await expect(page.locator(".customer-name")).toHaveText(
      "Corrected cloud name",
    );
    await page.reload();
    await page.getByText("Show changes", { exact: true }).click();
    await expect(
      page.getByRole("region", { name: "Customer changes" }),
    ).not.toContainText("Store owner");
    await expect(
      page.getByRole("region", { name: "Customer changes" }),
    ).toContainText("Fictional customer");
    await expect(
      page.getByText("Customer details edited", { exact: true }),
    ).toHaveCount(1);
...
test("scoped form balances and confirmation history refresh after a payment", async ({
  ...
  expect(calls.views).toEqual(
    expect.arrayContaining(["home", "directory", "customer"]),
  );
  expect(
    await page.evaluate(() => localStorage.getItem("tindahan.local-demo.v1")),
  ).toBeNull();
});
...
test("payment selection uses each customer balance and confirms the selected history", async ({
  ...
  state.customers.push({
    ...state.customers[0],
    id: second,
    name: "Second fictional customer",
  });
  state.entries.push({
    ...state.entries[0],
  ...
  await expect(
    page.getByText("Payment is higher than the remaining balance."),
  ).toBeVisible();
  await page.getByLabel("Customer", { exact: true }).selectOption(second);
  await expect(
    page.locator(".summary-row").filter({ hasText: "Current utang" }),
  ...
  await expect(page).toHaveURL(
    new RegExp(`confirmation\\?customer=${second}$`),
  );
  await expect(
    page
      .locator(".summary-row")
      .filter({ hasText: "Balance after this entry" }),
  ).toContainText("₱50.00");
...
test("scoped confirmations reload safely while legacy and mismatched links remain clear", async ({
  ...
  await page.reload();
  await expect(
    page.locator(".summary-row").filter({ hasText: "Previous balance" }),
  ...
  await expect(
    page.getByRole("heading", { name: "Entry not found" }),
  ).toBeVisible();
  expect(calls.fullReads).toBe(0);
  ...
  await expect(
    page.getByRole("heading", { name: "Voided entry" }),
  ).toBeVisible();
...
test("a failed scoped read retries without borrowing another screen’s cached totals", async ({
  ...
  await context.route("**/rest/v1/rpc/read_notebook", async (route) => {
    if (route.request().postDataJSON().p_view === "directory" && reject) {
      ...
      return route.fulfill({
        status: 503,
        json: { message: "Fictional read failure" },
      });
    }
    return route.fallback();
  });
  await page
    ...
    .click();
  await expect(
    page.getByRole("heading", { name: "Notebook unavailable" }),
  ).toBeVisible();
...
test("an unapplied scoped-read migration reports setup failure without a full-read fallback", async ({
  ...
  await context.route("**/rest/v1/rpc/read_notebook", (route) =>
    route.fulfill({
      status: 404,
      json: { code: "PGRST202", message: "Function missing" },
    }),
  );
  await page.goto("/home");
  await expect(
  ...
  await expect(page.getByRole("alert")).toContainText(
    "database setup has not been applied",
  );
  expect(calls.fullReads).toBe(0);
  expect(
    await page.evaluate(() => localStorage.getItem("tindahan.local-demo.v1")),
...
test("retries a committed full payment after a lost response and zero-balance refresh", async ({
  ...
  await expect(
    page.getByText("This customer is fully paid.", { exact: false }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Retry save" }).click();
  await expect(
    page.getByRole("heading", { name: "Payment Saved", exact: true }),
  ).toBeVisible();
...
test("retains a saved form on refresh failure and retries the read without another write", async ({
  ...
  await expect(
    page.getByText(
      "Your entry was saved, but the notebook could not refresh.",
      { exact: false },
    ),
  ).toBeVisible();
  await expect(page.getByLabel("Payment amount")).toHaveValue("100");
...
test("two stale tabs show one successful payment and one rejected payment", async ({
  ...
  await expect
    .poll(
      async () =>
        (await page
          .getByRole("heading", { name: "Payment Saved", exact: true })
          .count()) +
        (await other
          .getByRole("heading", { name: "Payment Saved", exact: true })
          .count()),
  ...
  await expect(
    loser.getByRole("heading", { name: "Payment Saved", exact: true }),
  ).toBeVisible();
