...
- Local validation: 51 unit/PostgreSQL tests passed after the final clock change; all 66 mobile/desktop browser regressions passed for the pagination change. The final head must also pass the full checked pull-request workflow before merging into `develop`.
