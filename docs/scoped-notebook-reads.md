...
- Transaction forms use the directory's complete current balance for the selected customer. They do not derive a balance from an empty or partial entry list. PostgreSQL still validates and serializes financial writes against the full ledger, including historical constraints; the local repository also retains full-ledger validation. Failed/uncertain saves retain their original payload and request ID. Confirmed saves refresh the directory before opening the customer's complete confirmation history.
